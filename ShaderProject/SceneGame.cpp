...
void SceneGame::Init()
{
	...
	m_pPlayer2UI->SetNameString(L"プレイヤー２");
...
void SceneGame::Update(float tick)
{
	...
	if (m_End)
	{
		if (IsKeyPress('L'))
		{
			SceneRoot::PlayMusic("Assets/Music/SE/決定ボタンを押す1.wav", true);
...
{
	m_MoveNum += num;
	std::wstring str = L"あと" + TextUI::intToFullWidthString(m_MoveNum);
...
void SceneGame::ChengeAction()
{
	...
	case SceneGame::E_ACTION_ROLL:
		...
		break;
	case SceneGame::E_ACTION_MOVE:
		str = L"あと" + TextUI::intToFullWidthString(m_MoveNum);
		GetObj<TextUI>("DiceNumUI")->SetString(str);
